...
Background execution 결과는 OCI Object Storage에 저장되며, output pane의 **Background Executions** 탭에서 상태와 완료 결과를 확인합니다. 장시간 SQL을 전환하기 전에 사용 권한, 예상 실행 시간과 database 부하를 확인하고 Object Storage 결과의 접근 권한과 보존·삭제 기준을 정합니다. 완료와 실패를 구분해 추적하며, 실패한 실행은 표시된 오류와 결과 출력을 바탕으로 조사합니다.
...
Log group과 invocation log의 IAM 접근은 조사 담당자에게 필요한 범위로 제한하고 업무·감사 요구에 맞춰 보존과 삭제 기준을 정합니다. 대표 성공·실패 호출의 `opc-request-id`를 확인하되, 조사 자료를 공유할 때는 log에 포함된 식별자와 민감한 요청 정보의 노출 여부를 점검합니다.
