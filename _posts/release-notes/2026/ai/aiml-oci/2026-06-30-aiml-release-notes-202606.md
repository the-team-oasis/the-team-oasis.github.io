...
기업 내부 VCN에서 model endpoint를 호출해야 하거나 인터넷 경유를 피해야 하는 AI inference workload라면, private endpoint를 사용해 보안 경계와 네트워크 경로를 통제할 수 있습니다.
...
Data Science Scheduler에서 interval-based schedule의 randomized start window를 설정할 수 있게 되었습니다. 활성화하면 첫 실행 시 지정된 window 안에서 random offset을 선택하고, 이후 실행은 첫 실행 시점을 기준으로 고정된 interval에 따라 수행됩니다.

여러 notebook job, pipeline, model 관련 작업이 같은 시각에 몰리는 환경에서는 시작 시점을 분산해 compute와 API의 resource contention을 완화할 수 있습니다.

### 동작 방식

예를 들어 start time이 09:10 UTC이고 random window duration이 30분이면 첫 실행은 09:10~09:40 UTC 사이에서 무작위로 선택됩니다.

Random window duration의 최소값은 30분이며, 설정한 schedule interval을 초과할 수 없습니다. Random start를 활성화하고 duration을 지정하지 않으면 configured interval의 절반이 기본 duration으로 적용됩니다. Schedule 생성 또는 업데이트 전에는 next five executions preview에서 실제 실행 window를 확인하는 것이 좋습니다.
